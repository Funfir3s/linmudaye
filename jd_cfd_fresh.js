...
[task_local]
#京喜财富岛合成生鲜
45 * * * * jd_cfd_fresh.js, tag=京喜财富岛合成生鲜, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jxcfd.png, enabled=true

================Loon==============
[Script]
cron "45 * * * *" script-path=jd_cfd_fresh.js,tag=京喜财富岛合成生鲜

===============Surge=================
京喜财富岛合成生鲜 = type=cron,cronexp="45 * * * *",wake-system=1,timeout=3600,script-path=jd_cfd_fresh.js

============小火箭=========
京喜财富岛合成生鲜 = type=cron,script-path=jd_cfd_fresh.js, cronexpr="45 * * * *", timeout=3600, enable=true
 */
...
function readShareCode() {
    return new Promise(async resolve => {
        $.get({url: `https://gitee.com/linmudaye/updateTeam/raw/master/cfd_hb.json`, 'timeout': 10000}, (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
