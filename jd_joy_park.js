...
脚本会默认帮lin助力开工位，如需关闭请添加变量，变量名：HELP_JOYPARK，变量值：false
          ...
          if (printLog) {
            ...
            for (let i = 0; i < data.data.activityJoyList.length; i++) {
              ...
              if (data.data.activityJoyList[i].level >= 30 && $.isNode()) {
                await notify.sendNotify(`${$.name} - 账号${$.index} - ${$.nickName}`, `【京东账号${$.index}】${$.nickName || $.UserName}\n当前等级: ${data.data.level}\n已达到单次最高等级奖励\n请尽快前往活动查看领取\n活动入口：京东极速版APP->汪汪乐园\n更多脚本->"https://github.com/lin/JD_tencent_scf"`);
                $.log(`\n开始解锁新场景...\n`);
                await doJoyRestart()
              }
            }
...
function getShareCode() {
  return new Promise(resolve => {
      $.get({
          url: "https://raw.fastgit.org/linmudaye/updateTeam/main/shareCodes/joypark.json",
          headers: {
