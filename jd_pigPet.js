...
京东金融养猪猪
活动入口：京东金融养猪猪，
脚本更新地址：https://github.com/linmudaye/linmudaye
...
[task_local]
京东金融养猪猪
12 0-23/6 * * * https://raw.githubusercontent.com/linmudaye/linmudaye/main/jd_pigPet.js, tag=京东金融养猪猪, enabled=true
================Loon==============
[Script]
cron "12 0-23/6 * * *" script-path=https://raw.githubusercontent.com/linmudaye/linmudaye/main/jd_pigPet.js,tag=摇钱树助力
===============Surge=================
京东金融养猪猪 = type=cron,cronexp="12 0-23/6 * * *",wake-system=1,timeout=20,script-path=https://raw.githubusercontent.com/linmudaye/linmudaye/main/jd_pigPet.js
============小火箭=========
京东金融养猪猪 = type=cron,script-path=https://raw.githubusercontent.com/linmudaye/linmudaye/main/jd_pigPet.js, cronexpr="12 0-23/6 * * *", timeout=3600, enable=true
*
*/
const $ = new Env('金融养猪');
...
let shareId = 'HhNhC8nOr0wHgkcO1_99tA'
$.shareCodes = [];
if ($.isNode()) {
...
!(async () => {
  ...
  if (process.env.PIGPETSHARECODE) {
    ...
    let res = await getAuthorShareCode('https://raw.githubusercontent.com/linmudaye/updateTeam/main/shareCodes/pigPet.json')
    if (!res) {
      res = await getAuthorShareCode('https://raw.fastgit.org/linmudaye/updateTeam/main/shareCodes/pigPet.json')
    }
    if (res){
  ...
  console.log(`\n======开始大转盘助力======\n`);
  $.helpId = await getAuthorShareCode('https://raw.fastgit.org/linmudaye/updateTeam/main/shareCodes/pig.json');
  $.shareCodes = [...$.shareCodes, ...($.helpId || [])]
  for (let j = 0; j < cookiesArr.length; j++) {
    cookie = cookiesArr[j];
    if ($.shareCodes && $.shareCodes.length) {
      console.log(`\n自己账号内部循环互助，有剩余次数再帮【linmudaye】助力\n`);
              ...
              if (data.resultData.resultCode === 0 && n < 5) {
                ...
                for (let i = 0; i < $.friends.length; i++) {
                  if ($.friends[i].status === 1) {
                    ...
                    if (!['linmudaye', 'xfa05'].includes($.name)) { //放过孩子吧TT
