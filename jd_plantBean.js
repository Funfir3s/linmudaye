...
=====================================Quantumult X=================================
[task_local]
30 7-21/2 * * * jd_plantBean.js, tag=种豆得豆, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdzd.png, enabled=true
=====================================Loon================================
[Script]
cron "30 7-21/2 * * *" script-path=jd_plantBean.js,tag=京东种豆得豆
======================================Surge==========================
京东种豆得豆 = type=cron,cronexp="30 7-21/2 * * *",wake-system=1,timeout=3600,script-path=jd_plantBean.js
====================================小火箭=============================
京东种豆得豆 = type=cron,script-path=jd_plantBean.js, cronexpr="30 7-21/2 * * *", timeout=3600, enable=true
*/
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `https://cdn.jsdelivr.net/gh/linmudaye/updateTeam@master/shareCodes/plant_bean.json`, timeout: 20000}, (err, resp, data) => {
      try {
        if (err) {
          //console.log(`${JSON.stringify(err)}`)
          //console.log(`${$.name} API请求失败，请检查网路重试`)
