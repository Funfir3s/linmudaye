...
!(async () => {
  ...
  $.authorTuanList = await getAuthorShareCode('https://raw.githubusercontent.com/linmudaye/updateTeam/master/shareCodes/jd_zz.json');
  if (!$.authorTuanList) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/linmudaye/updateTeam@master/shareCodes/jd_zz.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    $.authorTuanList = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/linmudaye/updateTeam@master/shareCodes/jd_zz.json') || [];
  }
  const temp = await getAuthorShareCode('https://raw.fastgit.org/linmudaye/updateTeam/main/shareCodes/jd_zz.json') || []
