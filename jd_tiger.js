...
!(async () => {
    ...
    let authorCode = []
    let res = await getAuthorShareCode('https://raw.githubusercontent.com/linmudaye/updateTeam/master/shareCodes/tiger.json')
    if (!res) {
        res = await getAuthorShareCode('https://raw.fastgit.org/linmudaye/updateTeam/master/shareCodes/tiger.json')
    }
    if (res) {
